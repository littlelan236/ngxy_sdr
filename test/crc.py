...
# 计算crc8校验码
def get_crc8_check_sum(
	message: bytes | bytearray | Iterable[int],
	init_crc: int = CRC8_INIT,
) -> int:
	payload = _normalize_data(message)
	crc = init_crc & 0xFF
	for byte in payload:
...
# 对一包数据进行crc8校验 最后一个byte是校验码 判断是否与除最后一个byte计算出的校验码一致
def verify_crc8_check_sum(message_with_crc: bytes | bytearray | Iterable[int]) -> bool:
	data = _normalize_data(message_with_crc)
	if len(data) <= 2:
		return False
	expected = get_crc8_check_sum(data[:-1], CRC8_INIT)
	return expected == data[-1]


# 在一包数据的末尾添加crc8校验码
def append_crc8_check_sum(message: bytes | bytearray | Iterable[int]) -> bytes:
	data = _normalize_data(message)
	crc = get_crc8_check_sum(data, CRC8_INIT)
	return data + bytes([crc])


# 计算crc16校验码
def get_crc16_check_sum(
	message: bytes | bytearray | Iterable[int],
	init_crc: int = CRC16_INIT,
) -> int:
	payload = _normalize_data(message)
	crc = init_crc & 0xFFFF
	for byte in payload:
...
if __name__ == "__main__":
    # 创建与 C++ 相同的初始数组
    packet = bytearray([0xA5, 0x06, 0x00, 0x00, 0x00, 0x06, 0x0A, 0x52,
                        0x4D, 0x32, 0x30, 0x32, 0x36, 0x00, 0x00])

    # 计算前4个字节的 CRC8 并写入第5个字节（索引4）
    crc8 = get_crc8_check_sum(packet[:4], CRC8_INIT)
    packet[4] = crc8

    # 计算前13个字节的 CRC16 并写入最后两字节（低字节在前）
    crc16 = get_crc16_check_sum(packet[:13], CRC16_INIT)
    packet[13] = crc16 & 0xFF        # 低字节
    packet[14] = (crc16 >> 8) & 0xFF # 高字节

    # 按 C++ 格式打印
    for i, b in enumerate(packet):
        print(f"0x{b:02X}", end=(" " if i != 14 else "\n"))